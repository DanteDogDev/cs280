...
public:
  ...
  enum OA_EXCEPTION {
    ...
    E_BAD_BOUNDARY,      // block address is not in a page or not on any
                         // block-boundary
    E_MULTIPLE_FREE,     // block has already been freed
    E_CORRUPTED_BLOCK    // block has been corrupted (pad bytes have been
                         // overwritten)
  };
...
struct OAConfig {
  ...
  OAConfig(
      bool UseCPPMemManager = false, unsigned ObjectsPerPage = DEFAULT_OBJECTS_PER_PAGE, unsigned MaxPages = DEFAULT_MAX_PAGES, bool DebugOn = false,
      unsigned PadBytes = 0, const HeaderBlockInfo& HBInfo = HeaderBlockInfo(), unsigned Alignment = 0
  )
  ...
  bool UseCPPMemManager_;         // by-pass the functionality of the OA and use
                                  // new/delete
  ...
  unsigned InterAlignSize_;       // number of alignment bytes required between
                                  // remaining blocks
};
